...
def main():
    ...
    parser.add_argument("--period", default="2y",
                        help="yfinance period for historical fetch (e.g. 3mo, 6mo, 1y, 2y)")
    ...
    try:
        ...
        if args.historical:
            ...
            historical = fetcher.fetch_historical_prices(period=args.period)
            ...
            # Default: price pull; capture prices to avoid duplicate fetch
            _current_prices = fetcher.run_price_pull()

        # Always update paper portfolios if prices changed
        if args.update_paper or not (args.fundamentals or args.sync):
            try:
                prices_df = _current_prices if _current_prices is not None else fetcher.fetch_current_prices()
